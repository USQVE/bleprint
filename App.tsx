...
import { Database, Play, RotateCcw, Trash2, Plus, Minus, MousePointer2, X, Terminal, ZoomIn, ZoomOut, ListTree, Copy, Check } from 'lucide-react';

import { CommentBox as CommentBoxType, Connection as ConnectionType, NodeData } from './types';
import { parseUniversal, buildAsciiTreeExec } from './utils/parser';
...
const App: React.FC = () => {
  ...
  const applyGraph = useCallback(() => {
    try {
      const { nodes: parsedNodes, connections: parsedConns } = parseUniversal(inputText);
      setNodes(parsedNodes);
      setConnections(parsedConns);
      setTreeText(buildAsciiTreeExec(parsedNodes, parsedConns));
      setCommentBoxes([
        ...
        <div style={{
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ padding: '10px 24px', fontSize: 9, fontWeight: 900, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Source Code (Clipboard/Arrows)</div>
              <textarea
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.2)' }}>
              <div style={{ padding: '10px 24px', fontSize: 9, fontWeight: 900, color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><ListTree size={12} /> Execution Logic Tree</span>
